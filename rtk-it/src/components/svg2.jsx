...
// Создаем глобальную ссылку для WebSocket соединения
...
const Svg = () => {
  ...
  // Функция для установки подписок WebSocket
  ...
  const getWebSocketConnection = useCallback(async () => {
    // Если соединение уже установлено и активно
  ...
  // Подключение к WebSocket для реальных данных (однократно)
  useEffect(() => {
    const componentId = componentIdRef.current;

    // Добавляем компонент в список подписчиков
    ...
    getWebSocketConnection()
      .then((stompClient) => {
        // Если соединение уже активно, настраиваем подписки сразу
    ...
    return () => {
      ...
      // Если это последний подписчик и соединение активно, можно закрыть соединение
      // Но лучше оставить глобальное соединение активным для других компонентов
      // if (connectionSubscribers.size === 0 && globalStompClient) {
